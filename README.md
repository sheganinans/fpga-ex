...
Note: If you need a newer Clash version, simply change the version bounds in `freq-div.cabal` and follow the hints given by Stack.

## src/
This is where the source code of the project lives, as specified in `freq-div.cabal`. It contains a single file, `Example/Project.hs` which starts with:
...
`freq-div.cabal` enabled `NoImplicitPrelude` which enables the use of `Clash.Prelude` here. Next, a function `plus` is defined. It simply adds two numbers. Note that the example (`>>> plus 3 5`) gets executed by the _doctests_ defined for this project and checked for consistency with the result in the documentation (`8`).
